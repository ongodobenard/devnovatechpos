...
export default function Hardware() {
  ...
  const bizName = user?.business_name || 'Hardware Store'

  const [tab, setTab] = useState(() => localStorage.getItem('pos_tab_hw') || (isAdmin ? 'dashboard' : 'pos'))
  ...
  const handleNavClick = id => { setTab(id); localStorage.setItem('pos_tab_hw', id); if (isMobile) setMobileNavOpen(false) }
  ...
  const buildReceiptHTML = (txnNo, now, tendered, chg, pm, custName = null) => {
    ...
    return `<div style="font-family:'Courier New',monospace;font-size:10px;color:#000;max-width:300px;margin:0 auto;">
      <div style="text-align:center;margin-bottom:6px;">
        ...
        <div style="font-size:9px;color:#555;">HARDWARE POS</div>
  ...
  const buildReprintHTML = (sale) => {
    ...
    return `<div style="font-family:'Courier New',monospace;font-size:10px;color:#000;max-width:300px;margin:0 auto;">
      <div style="text-align:center;margin-bottom:6px;">
        <div style="font-size:14px;font-weight:700;">${bizName.toUpperCase()}</div>
        <div style="font-size:9px;color:#555;">HARDWARE POS - REPRINT</div>
        <div style="font-size:9px;color:#555;">${ds} ${ts}</div>
            ...
            <div className="pnav" style={{ width: sidebarOpen ? '185px' : '46px' }}>
              <div className="pnav-logo">
                ...
                {sidebarOpen && <div style={{ overflow: 'hidden' }}>
                  ...
                  <div style={{ fontSize: 8, letterSpacing: 2, color: '#8BAAC8' }}>HARDWARE POS</div>
                </div>}
              </div>
              <div className="pnav-items">
                {NAV.map(n => (
                  <div key={n.id} className={`pnav-item${tab === n.id ? ' active' : ''}`} onClick={() => { setTab(n.id); localStorage.setItem('pos_tab_hw', n.id) }}>
            ...
            <form id="prodform" onSubmit={handleSaveProd} style={{ padding: '12px 15px', overflowY: 'auto', maxHeight: '65vh' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 9 }}>
                <div style={{ gridColumn: '1/-1' }}>
                  ...
                  <input style={inp} type="text" required placeholder="e.g. DeWalt 20V Drill" value={prodForm.name} onChange={e => setProdForm(p => ({ ...p, name: e.target.value }))} />
                </div>
                ...
                {[
                  { label: 'Model / SKU', key: 'generic_name', type: 'text', ph: 'e.g. DCD771C2' },
        ...
        <div className="povl open" onClick={e => e.target === e.currentTarget && setShowAddCat(false)}>
          <div className="pmod" style={{ maxWidth: 360 }}>
            ...
            <form id="catform" onSubmit={handleAddCategory} style={{ padding: '12px 15px' }}>
              ...
              <input style={inp} type="text" required placeholder="e.g. Hammer" value={catForm.name} onChange={e => setCatForm({ name: e.target.value })} />
