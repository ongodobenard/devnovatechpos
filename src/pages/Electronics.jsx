...
const PATHS = {
  ...
  device: 'M5 6h14a2 2 0 012 2v8a2 2 0 01-2 2H5a2 2 0 01-2-2V8a2 2 0 012-2z||M8 21h8',
...
export default function Electronics() {
  ...
  const bizName = user?.business_name || 'Electronics Store'

  const [tab, setTab] = useState(() => localStorage.getItem('pos_tab_elec') || (isAdmin ? 'dashboard' : 'pos'))
  ...
  const [prodForm, setProdForm] = useState({ name: '', generic_name: '', category: '', buying_price: '', selling_price: '', profit_margin: '', apply_tax: false, stock_quantity: '', reorder_level: '10', unit: '', barcode: '' })
  ...
  const handleNavClick = id => { setTab(id); localStorage.setItem('pos_tab_elec', id); if (isMobile) setMobileNavOpen(false) }
  ...
  const handleCSVImport = async (e) => {
    ...
    try {
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        try { await productsAPI.add({ ...mapped, business_id: bizId }); success++ }
        catch (err) { failed++; errors.push(`Row ${i + 2} (${mapped.name}): ${err.response?.data?.error || 'Failed'}`) }
      }
      setCsvFeedback({ success, failed, errors, total: rows.length })
      setShowCsvFeedback(true)
      if (success > 0) fetchProducts()
    } catch (err) { alert('Failed to read CSV file.') }
  ...
  const fetchAuditLogs = async () => {
    ...
    try {
      ...
      const saleLogs = (r.data.data || []).map(s => ({
        id: `sale-${s.id}`, action: 'SALE',
        description: `Sale ${s.receipt_number} - ${fmtKES(s.total_amount)}`,
        user: s.cashier_name || '-', timestamp: s.created_at, meta: s.payment_method, type: 'sale'
      }))
      setAuditLogs(saleLogs)
    } catch (e) { setAuditLogs([]) } finally { setLoadingAudit(false) }
  }
  ...
  const openAddProd = () => {
    setEditProd(null)
    setProdForm({ name: '', generic_name: '', category: '', buying_price: '', selling_price: '', profit_margin: '', apply_tax: false, stock_quantity: '', reorder_level: '10', unit: '', barcode: '' })
    setProdError(''); setShowAddProd(true)
  }

  const openEditProd = p => {
    setEditProd(p)
    setProdForm({ name: p.name, generic_name: p.generic_name || '', category: p.category || '', buying_price: p.buying_price, selling_price: p.selling_price, profit_margin: '', apply_tax: false, stock_quantity: p.stock_quantity, reorder_level: p.reorder_level, unit: p.unit || '', barcode: p.barcode || '' })
    setProdError(''); setShowAddProd(true)
  }
  ...
  const fetchReport = async () => {
    ...
    } catch (e) { setReportData(null) } finally { setLoadingReport(false) }
  }
  ...
  const fetchSales = async () => {
    ...
    try {
      let data = []
      if (isAdmin) { const r = await salesAPI.listAll(bizId, salesDateFrom, salesDateTo); data = r.data.data || [] }
      else { const r = await salesAPI.list(bizId, todayStr()); data = (r.data.data || []).filter(s => String(s.cashier_id) === String(user.id)) }
      setSales(data)
  ...
  const buildReceiptHTML = (txnNo, now, tendered, chg, pm, custName = null) => {
    ...
    return `<div style="font-family:'Courier New',monospace;font-size:10px;color:#000;max-width:300px;margin:0 auto;">
      <div style="text-align:center;margin-bottom:6px;">
        ...
        <div style="font-size:9px;color:#555;">ELECTRONICS POS</div>
  ...
  const buildReprintHTML = (sale) => {
    ...
    return `<div style="font-family:'Courier New',monospace;font-size:10px;color:#000;max-width:300px;margin:0 auto;">
      <div style="text-align:center;margin-bottom:6px;">
        <div style="font-size:14px;font-weight:700;">${bizName.toUpperCase()}</div>
        <div style="font-size:9px;color:#555;">ELECTRONICS POS - REPRINT</div>
        <div style="font-size:9px;color:#555;">${ds} ${ts}</div>
  ...
  const handleReturn = (sale) => {
    if (!isAdmin) { setPendingRefundSale(sale); setAdminPw(''); setShowAdminPwModal(true) }
    else processReturn(sale, '')
  }
  ...
  const processReturn = async (sale, adminPassword) => {
    ...
    try {
      ...
      fetchSales(); fetchTodaySales(); if (isAdmin) fetchStats()
    } catch (e) { alert(e.response?.data?.error || 'Return failed') }
  }

  const handleAdminPwSubmit = async (e) => {
    e.preventDefault(); setShowAdminPwModal(false)
    await processReturn(pendingRefundSale, adminPw); setAdminPw('')
  }
    ...
    <>
      <style>{`
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
        ...
        .pos-list-table thead th{background:#F9FAFB;font-size:10px;font-weight:700;color:#9AA3B0;text-transform:uppercase;padding:8px 10px;border-bottom:1px solid #E5E7EB;text-align:left;white-space:nowrap;}
            ...
            <div className="pnav" style={{ width: sidebarOpen ? '185px' : '46px' }}>
              <div className="pnav-logo">
                ...
                {sidebarOpen && <div style={{ overflow: 'hidden' }}>
                  ...
                  <div style={{ fontSize: 8, letterSpacing: 2, color: '#8BAAC8' }}>ELECTRONICS POS</div>
                </div>}
              </div>
              <div className="pnav-items">
                {NAV.map(n => (
                  <div key={n.id} className={`pnav-item${tab === n.id ? ' active' : ''}`} onClick={() => { setTab(n.id); localStorage.setItem('pos_tab_elec', n.id) }}>
                ...
                {tab === 'pos' && (
                  <div className="pos-wrap" style={{ height: isMobile ? 'auto' : 'calc(100vh - 168px)' }}>
                    <div className="pos-left">
                      ...
                      {/* RESPONSIVE PRODUCT TABLE — same pattern as Sales History, NOT the old pharmacy flex version */}
                      <div className="tbl-wrap" style={{ background: '#fff', borderRadius: 8, border: '1px solid #E5E7EB', maxHeight: isMobile ? '340px' : 'calc(100vh - 280px)' }}>
                            ...
                            {posProducts.length === 0
                              ...
                              : posProducts.map(p => (
                                <tr key={p.id} className={p.stock_quantity < 1 ? 'out-row' : ''} onClick={() => addToCart(p)}>
                                  <td>
                                    <div style={{ fontWeight: 600, color: '#1E3A5F', fontSize: 11 }}>{p.name}</div>
                                    <div style={{ fontSize: 9, color: '#9AA3B0' }}>{p.generic_name || '—'}</div>
                                  </td>
                    ...
                    {loadingProds ? <div className="pempty">Loading…</div> : filteredProds.length === 0 ? <div className="pempty">No products found.</div> : (
                      <div className="tbl-wrap">
                        <table className="pt" style={{ minWidth: 500 }}>
                          <thead><tr><th>#</th><th>Product</th><th>Category</th><th>Buying</th><th>Selling</th><th>Stock</th>{isAdmin && <th>Actions</th>}</tr></thead>
                          <tbody>
            ...
            <form id="prodform" onSubmit={handleSaveProd} style={{ padding: '12px 15px', overflowY: 'auto', maxHeight: '65vh' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 9 }}>
                <div style={{ gridColumn: '1/-1' }}>
                  ...
                  <input style={inp} type="text" required placeholder="e.g. Samsung 55in Smart TV" value={prodForm.name} onChange={e => setProdForm(p => ({ ...p, name: e.target.value }))} />
                </div>
                ...
                {[
                  { label: 'Model / SKU', key: 'generic_name', type: 'text', ph: 'e.g. UA55AU7000' },
                  { label: 'Unit', key: 'unit', type: 'text', ph: 'e.g. pcs' },
                  ...
                  <div style={{ gridColumn: '1/-1', marginTop: 8 }}>
                    ...
                    <div style={{ marginTop: 8, padding: '7px 10px', background: '#F8FAFF', border: '1px solid #DBEAFE', borderRadius: 6 }}>
                      ...
                      <div style={{ fontSize: 10, color: '#6B7280', fontFamily: 'monospace', lineHeight: 1.6 }}>
                        Name, Category, Buying Price, Selling Price, Stock Quantity, VAT*
                      </div>
                      <div style={{ fontSize: 9, color: '#9CA3AF', marginTop: 3 }}>* VAT is optional - leave blank if not applicable</div>
                    </div>
        ...
        <div className="povl open" onClick={e => e.target === e.currentTarget && setShowAddCat(false)}>
          <div className="pmod" style={{ maxWidth: 360 }}>
            ...
            <form id="catform" onSubmit={handleAddCategory} style={{ padding: '12px 15px' }}>
              ...
              <input style={inp} type="text" required placeholder="e.g. Televisions" value={catForm.name} onChange={e => setCatForm({ name: e.target.value })} />
        ...
        <div className="povl open" onClick={e => e.target === e.currentTarget && setShowAddCash(false)}>
          <div className="pmod" style={{ maxWidth: 390 }}>
            ...
            <form id="cashform" onSubmit={handleAddCashier} style={{ padding: '12px 15px' }}>
              ...
              {[{ label: 'Full Name *', key: 'name', type: 'text', ph: 'e.g. Jane Doe', req: true }, { label: 'Email *', key: 'email', type: 'email', ph: 'jane@store.com', req: true }, { label: 'Password *', key: 'password', type: 'password', ph: 'Min 8 characters', req: true }].map(f => (
